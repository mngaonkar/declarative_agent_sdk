...
from declarative_agent_sdk.runtime.llm import LeanLLMClient
...
class LeanAIAgent(BaseAgent):
    """
    Host-side agent using the lean ReAct + progressive-skills runtime.

    Same BaseAgent surface as AIAgent / LangChainAIAgent so Discord and A2A
    ...
    def __init__(
        ...
        skills: Optional[List[str]] = None,  # unused — lean discovers all under skills_directory
        ...
        for fn in resolved:
            # Avoid shadowing core skill tools
            if getattr(fn, "__name__", "") in ("read_file", "write_file"):
                # Prefer lean FS tools for skill authoring; still expose builtins
                # under their names only if not already registered.
                pass
            try:
